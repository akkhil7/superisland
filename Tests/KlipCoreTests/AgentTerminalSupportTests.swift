...
final class AgentTerminalSupportTests: XCTestCase {
    ...
    func testParseFullVSCodeTitle() {
        let parsed = EditorWindowTitle.parse("● main.swift — useklip — Visual Studio Code")
        XCTAssertEqual(parsed.fileName, "main.swift")
        XCTAssertEqual(parsed.workspaceName, "useklip")
        XCTAssertTrue(parsed.isDirty)
    }

    func testParseTitleWithoutAppName() {
        let parsed = EditorWindowTitle.parse("main.swift — useklip")
        XCTAssertEqual(parsed.fileName, "main.swift")
        XCTAssertEqual(parsed.workspaceName, "useklip")
    ...
    func testParseWorkspaceOnlyTitle() {
        let parsed = EditorWindowTitle.parse("useklip — Cursor")
