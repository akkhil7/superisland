...
final class ContextAnchorTests: XCTestCase {
    ...
    func testNoteReasonKeepsStatusAndDeduplicates() throws {
        ...
        let store = DropStore(fileURL: dir.appendingPathComponent("drops.json"))
        ...
        let drop = Drop(label: "L", target: target, status: .working)
        store.add(drop)

        store.noteReason(id: drop.id, reason: "In a background tab")
        store.noteReason(id: drop.id, reason: "In a background tab")
        store.noteReason(id: drop.id, reason: "In a background tab")

        let stored = try XCTUnwrap(store.drop(id: drop.id))
