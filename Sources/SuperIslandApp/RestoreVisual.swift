...
enum RestoreAnchorCollector {
                ...
                let anchors = observations.enumerated().compactMap { idx, observation -> CollectedRestoreAnchor? in
                    ...
                    return CollectedRestoreAnchor(
                        anchor: RestoreAnchor(
                            ...
                            role: "OCRText",
                            label: text,
                            frame: SuperIslandCore.NormalizedRect(
                                x: box.origin.x,
                                y: box.origin.y,
                                width: box.width,
                                height: box.height
    ...
    static func normalize(_ rect: CGRect, in windowFrame: CGRect) -> SuperIslandCore.NormalizedRect {
        ...
        return SuperIslandCore.NormalizedRect(
...
final class RestoreMemoryStore {
    ...
    func save(memory: RestoreMemory, screenshotPNG: Data?) throws {
        ...
        guard let combined = sealed.combined else {
            throw NSError(domain: "DropRestoreMemory", code: -1)
        }
    ...
    private func url(for id: UUID) -> URL {
        directory.appendingPathComponent("\(id.uuidString).droprestore")
    }
    ...
    private func key() throws -> SymmetricKey {
        ...
        guard status == errSecSuccess else {
            throw NSError(domain: "DropRestoreMemory", code: Int(status))
        }
        let data = Data(bytes)
