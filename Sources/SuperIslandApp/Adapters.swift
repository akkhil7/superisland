...
/// Per-app strategy for capturing a precise `Locator` at drop time and for
...
protocol AppAdapter {
    ...
    /// Build the locator for the just-dropped front window.
...
/// CGWindowID captured at drop time.
struct GenericAXAdapter: AppAdapter {
    ...
    /// tab/section that was selected when the drop was dropped. Runs slightly
...
struct ChromeAdapter: AppAdapter {
    ...
    func captureLocator(front: FrontWindow) -> Locator {
        // AppleScript reads the ACTUAL active tab at drop time — ground truth.
...
///
/// Two kinds of drops, picked by where keyboard focus is at drop time:
...
// MARK: - Shell (TTY-based refocus for shell-integration drops)
...
/// Entry point used by the UI: route a drop to the adapter that owns it.
enum Refocuser {
    @discardableResult
    static func refocus(_ drop: Drop) -> Bool {
        if case .shell = drop.target.locator {
            return ShellAdapter().refocus(target: drop.target)
        }
        return AdapterRegistry.adapter(for: drop.target.bundleID).refocus(target: drop.target)
    }
}
