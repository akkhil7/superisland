...
/// `~/.codex/sessions/`. SuperIsland watches those: the freshest rollout identifies
/// the thread you just prompted (drop-time binding), its appended events
/// drive drop status, and `codex://threads/<id>` deep-links back to it.
@MainActor
final class CodexIntegration: ObservableObject {
    static let bundleID = CodexDeepLink.bundleID
    /// Pseudo-URL stored in `WindowTarget.contentURL` for Codex drops.
    ...
    /// The thread a freshly dropped drop belongs to. The desktop app keeps
    ...
    /// to bind a `codex` CLI launch inside a dropped terminal to its session.
