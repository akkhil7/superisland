...
/// SuperIsland-status semantics for Codex lifecycle events (same stdin payload shape
...
public enum CodexHookMapper {
    ...
    public static let commandMarker = "superisland-codex-hook"
}
...
/// `event_msg` lines live. Watching those files is SuperIsland's event source.
public enum CodexRollout {
    ...
    public static func latestUpdate(fromTail tail: String) -> ClaudeHookMapper.Update? {
        ...
        for line in tail.split(separator: "\n") {
            ...
            case let k where k.hasSuffix("_begin"):
                // The agent started executing something (e.g. running a
                // command you just approved: `exec_command_begin`,
                // `patch_apply_begin`). Approval *requests* end in `_request`,
                // so this only fires once Codex has resumed.
                last = .init(status: .working, reason: "Codex is working…")
            case "task_complete":
...
/// switch (verified live). SuperIsland binds drops to the freshest rollout *within*
/// that workspace.
