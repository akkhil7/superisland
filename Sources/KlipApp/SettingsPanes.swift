            ...
            Section {
                Toggle(isOn: $settings.useScreenshots) {
                    SettingsRowLabel(
                        ...
                        subtitle: "Lets Klip read windows that expose no text (many Electron apps). Sent to Claude only for your klips. Needs Screen Recording."
                    )
                }
            }
        }
...
struct IntegrationsSettingsPane: View {
    ...
    private var claudeCard: some View {
        IntegrationCard(
            ...
            Text("Live status for Claude Code and Cowork sessions via Claude's own hooks: the instant Claude finishes or needs you, your klip updates — even in background tabs. No AI calls.")
                .settingsCaption()
    ...
    private var otherAppsCard: some View {
        IntegrationCard(
            ...
            Text("Apps without a deep integration get encrypted, local-only visual memory: Klip remembers what your task looked like and highlights it when you return — always confirm-before-click.")
                .settingsCaption()
        ...
        Form {
            Section {
                permissionRow(
                    ...
                    title: "Accessibility",
                    subtitle: "Reads window text and raises the exact window when you click a klip. Required.",
                ...
                Text("Toggle ON in System Settings but Klip still shows “not granted”? The grant went stale after a rebuild — click ⟲ to reset Klip's permission entry and get a fresh prompt.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
            }
            ...
            Section {
                HStack {
                    SettingsRowLabel(
                        ...
                        title: "Automation",
                        subtitle: "Lets Klip select exact Chrome tabs and terminal panes via Apple Events. macOS asks per-app on first use."
                    )
                    Spacer()
    ...
    private func permissionRow(
        ...
        HStack {
            ...
            if granted {
                ...
                .controlSize(.small)
                .help("Reset Klip's \(title) entry and re-prompt (fixes a stale grant after a rebuild)")
            }
        }
    }
}
...
struct AboutSettingsPane: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Link("Send Feedback", destination: URL(string: "mailto:hi@useklip.com")!)
                .font(.caption)
